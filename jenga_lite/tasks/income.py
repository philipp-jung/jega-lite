...
class IncomeEstimationTask(BinaryClassificationTask):
    ...
    def __init__(self, seed, ignore_incomplete_records_for_training=False) -> None:
        ...
        ----
            ...
            ignore_incomplete_records_for_training (bool, optional): Indicates whether or not to drop rows with missing values for training. \
                Defaults to False.
        """
        columns = ["workclass", "occupation", "marital-status", "education", "hours-per-week", "age"]
        categorical_columns = ["workclass", "occupation", "marital-status", "education"]
        numerical_columns = ["hours-per-week", "age"]

        all_data = fetch_openml(data_id=1590, as_frame=True)["frame"]
